      ...
      >
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-white/40 px-4 py-3">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-stone-900">Choose delivery location</p>
            {activeAddress ? (
              <p className="truncate text-xs text-brand-700" title={activeAddress}>
                Active: {activeAddress}
              </p>
            ) : (
              <p className="text-xs text-stone-500">Search for your delivery area</p>
            )}
          </div>
          <button type="button" onClick={onClose} className="glass-icon-btn h-8 w-8 shrink-0 text-stone-500" aria-label="Close">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          <button
            ...
            className="mt-3 flex w-full items-center gap-3 rounded-2xl border border-white/55 bg-white/45 px-3 py-2.5 text-left text-sm font-medium text-stone-800 transition hover:bg-white/70"
          >
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-brand-700 [&_svg]:h-4 [&_svg]:w-4">
              {customerIcons.location}
            </span>
            <span className="min-w-0">
              <span className="block font-semibold">Use current location</span>
              <span className="block text-xs font-normal text-stone-500">Detect automatically using GPS</span>
            </span>
          </button>
          ...
          {!preview && query.trim().length < MIN_SEARCH_LENGTH && (
            <div className="mt-[17px] flex justify-end">
              <button
                ...
                className="text-sm font-semibold uppercase tracking-wide text-brand-700 transition hover:text-brand-800"
              >
          ...
          {recentSearches.length > 0 && !preview && query.trim().length < MIN_SEARCH_LENGTH && (
            <div className="mt-4">
              <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-stone-500">Recent searches</p>
              <ul className="space-y-2">
                {recentSearches.map((item) => (
                  <li key={item.label}>
                    ...
                    >
                      <span className="line-clamp-2 text-sm text-stone-800">{item.label}</span>
                    </button>
                  </li>
                ))}
              </ul>
          ...
          {query.trim().length >= MIN_SEARCH_LENGTH && !preview && (
            <div className="mt-4">
              <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-stone-500">Suggestions</p>
          ...
          {preview && (
            <div className="mt-4 space-y-3">
              ...
              />
              <p className="line-clamp-2 text-sm text-stone-700">{preview.label}</p>
              <div className="flex flex-wrap gap-2">
                <button type="button" className="btn-primary" onClick={() => confirmSelection(preview)}>
                  Confirm location
                </button>
